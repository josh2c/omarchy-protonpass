...
> **This file is not a changelog.** It is the frozen record of how v1 was planned and built.
> Do not append release notes to it. Release notes belong in the annotated release tags.
> This document lives on the `dev-docs` branch and is not shipped to users; the plugin tree on
> `main` carries only the code, tests, README, LICENSE, and preview.
...
**Rev 4.4 (field defect F2 + release-gate note):** the 1.2.0 candidate's refresh button set `iconSpinning` — a `qs.Ui Button` property — on `PanelActionButton`, which lacks it; QML component instantiation fails at runtime and the bar silently drops the widget (observed live; two coincident quickshell SIGSEGVs during reload are tracked separately as a suspected upstream issue). Fix: the property line removed — the disabled state and footer spinner already convey refreshing. Lesson encoded: CI cannot instantiate `qs.Ui` components (shell-only imports), so qmllint green ≠ loadable; the acceptance checklist's fresh-install step is the authoritative instantiation gate, and any new use of a `qs.Ui` component property must be verified against `/usr/share/omarchy/shell/Ui/` source before commit.
