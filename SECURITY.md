...
- **The one-way hash at rest.** To auto-clear safely, the helper keeps a SHA-256 hash of the last copied value in a user-only runtime file. A hash cannot be reversed, but a *low-entropy* value can be guessed offline from it: a 6-digit TOTP is 10⁶ candidates, which an ordinary CPU exhausts in well under a second (measured: ~0.06 s, roughly 18M SHA-256 per second). The file holds nothing else, and it is deleted whenever the clipboard is cleared. That again takes an attacker already on your user account, who could read the live clipboard anyway. Strong generated passwords are unaffected. The hash is kept for TOTP on purpose: it is what lets the helper confirm the clipboard still holds the code before clearing it, and skipping it would leave short-lived codes on the clipboard for good, the worse of the two trades.
